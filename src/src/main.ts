...
class Terminal {
  ...
  private minimizeTerminal() {
    ...
    setTimeout(() => {
      ...
      `
      // Start the clock
      if (window.taskbarClockInterval) clearInterval(window.taskbarClockInterval)
      const updateClock = () => {
  ...
  private completeCommand(textNode: Text, input: string) {
    const matches = this.commands.filter(cmd => cmd.toLowerCase().startsWith(input))
  ...
  private completeArgument(
    ...
    const matches = validArgs.filter(arg => arg.toLowerCase().startsWith(currentArg))
  ...
  private async startSnakeGame() {
    ...
    Array.from(this.outputContainer.querySelectorAll('.snake-terminal-container')).forEach(el =>
      el.remove()
    )
